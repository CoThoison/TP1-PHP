...
class ASTSQLQueryVisitor:
    ...
    def __visit(self, node_id: int):
        if self.ast.get_type(node_id) == "FunctionCall" and  (self.ast.get_image(node_id) == "mysql_query" or self.ast.get_image(node_id) == "mysqli_query"):
            print(f"SQL query is called "
                  f"at line {self.ast.get_position(node_id)[0]}")
        if self.ast.get_type(node_id) == "MethodCall" and  (self.ast.get_image(node_id) == "execute" or self.ast.get_image(node_id) == "exec"):
            print(f"SQL query is called "
                  f"at line {self.ast.get_position(node_id)[0]}")
...
visitor.visit(ast)
visitor.visit(ast_2)
# Copy of a file in which we assume a SQL query is called, MySQL.php in wordpress sources
visitor.visit(ast_3)
print("\n")
